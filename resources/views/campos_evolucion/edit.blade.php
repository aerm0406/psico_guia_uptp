    ...
    <div class="py-12 bg-slate-50 dark:bg-gray-900 min-h-screen">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="mb-8 flex items-center gap-4">
                ...
                <div>
                    <h2 class="text-3xl font-black text-slate-900 dark:text-white tracking-tight">Editar Campo</h2>
                    <p class="text-sm font-bold text-slate-500 dark:text-gray-400 mt-1 uppercase tracking-widest">{{ $campo->titulo }}</p>
                </div>
            </div>

            <div class="bg-white dark:bg-gray-800 rounded-[32px] p-8 shadow-sm border border-slate-100 dark:border-gray-700 max-w-2xl">
                <form action="{{ route('campos-evolucion.update', $campo->id) }}" method="POST">
                    @csrf
                    @method('PATCH')
